define(["../my-app.js"],function(_myApp){"use strict";class Ksm extends _myApp.PolymerElement{static get template(){return _myApp.html`
    ...
    `}static get properties(){return{subKategori:{type:Array,notify:!0},user:{type:Array,notify:!0},regObj:{type:Object,notify:!0,value:function(){return{judul_proposal:"-",kategoris:{jumlah_bantuan:"0"},persetujuan:{manager_id:"-"},tanggalProposal:"000-00-00"}}}}}static get observers(){return["_changeStoI(regObj.kategoris.*)","_changeDate(regObj.tanggalProposal)"]}_changeStoI(f){var array=f.path.split(".");if("jumlah_bantuan"==array[2]){f.base[array[2]]=parseInt(f.value)}}_changeDate(f){if(""!==f&&"undefined"!==typeof f){var date=this.$.tanggal_proposal,that=this;date.value=this.formatDate(new Date(f));date.addEventListener("change",function(){if(""!==date.value){that.regObj.tanggalProposal=new Date(date.value).toISOString();console.log(that.regObj.tanggalProposal)}})}}formatDate(date){var dd=date.getDate(),mm=date.getMonth()+1,yyyy=date.getFullYear();return yyyy+"-"+mm+"-"+dd}}window.customElements.define("bmm-kategori-ksm",Ksm)});