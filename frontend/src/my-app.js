...
`}static get is(){return"vaadin-item"}static get version(){return"2.1.0"}}_exports.ItemElement=ItemElement;customElements.define(ItemElement.is,ItemElement);var vaadinItem={ItemElement:ItemElement};_exports.$vaadinItem=vaadinItem;const $_documentContainer$q=document.createElement("template");$_documentContainer$q.innerHTML=`<dom-module id="lumo-item" theme-for="vaadin-item">
  <template>
    <style>
      :host {
        display: flex;
...
</dom-module>`;document.head.appendChild($_documentContainer$u.content);const $_documentContainer$v=document.createElement("template");$_documentContainer$v.innerHTML=`<dom-module id="vaadin-number-field-template">
  <template>
    <style>
      :host([readonly]) {
        pointer-events: none;
...
</dom-module>`;document.head.appendChild($_documentContainer$w.content);const $_documentContainer$x=html`<dom-module id="lumo-text-area" theme-for="vaadin-text-area">
  <template>
    ...
    `}disconnectedCallback(){super.disconnectedCallback();globalData.unsubscribe(this.key,this)}_notifyOrigin(key,readonly){return newValue=>{if(!readonly){globalData.set(key,newValue)}}}_subscribe(newKey,oldKey){if(newKey!=oldKey&&oldKey){globalData.unsubscribe(oldKey,this)}globalData.subscribe(newKey,this);this.set("value",globalData.get(newKey))}_onEvent(event,detail){const{SET}=globalData.EVENTS;if(event===SET){this._set(detail)}}_set(value){this.set("value",value.value)}}customElements.define("global-variable",GlobalVariable);const $_documentContainer$y=document.createElement("template");$_documentContainer$y.innerHTML=`<dom-module id="custom-css">
  <template>
    <style>
...
</dom-module>`;document.head.appendChild($_documentContainer$y.content);const $_documentContainer$z=document.createElement("template");$_documentContainer$z.innerHTML=`<iron-iconset-svg name="my-icons" size="24">
  <svg>
    <defs>
      <g id="arrow-back">
...
</iron-iconset-svg>`;document.head.appendChild($_documentContainer$z.content);setPassiveTouchGestures(!0);setRootPath(MyAppGlobals.rootPath);class MyApp extends PolymerElement{static get template(){return html`
