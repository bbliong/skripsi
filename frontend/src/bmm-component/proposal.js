...
connectedCallback() {
  ...
  _clicked(){
    ...
    action.renderer = function(root, column, rowData){
      ...
      if(  that.storedUser.role == 3 && rowData.item.persetujuan.level_persetujuan >= 5 && rowData.item.persetujuan.level_persetujuan !== 6) {
        let el = document.createElement("paper-button")
        el.innerHTML = "Komite"
        el.classList.add("blue")
        root.appendChild(el)
        root.childNodes[4].addEventListener('click', function(e){
          that.set('route.path', '/panel/proposal/komite-manager/'+ rowData.item.kategori +  "/" +  rowData.item._id );
        })
        action.width = "240px"
    }
    ...
    this.$.status.renderer = (root, grid, rowData) => {
      ...
      switch(rowData.item.persetujuan.level_persetujuan){
        ...
        case  4 :
            ...
            break;
        case  5 :
            status = "UPD Disetujui Kadiv"
            root.classList.add("status-verifikasi")
            root.style.backgroundColor = colors[5] 
            break;
        case  6 :
            status = "UPD Tidak disetujui Kadiv"
            root.classList.add("status-verifikasi")
